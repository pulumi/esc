...
// RotateExpr is a function expression that invokes an environment provider by name.
...
func parseShortRotate(node *syntax.ObjectNode, name *StringExpr, args Expr) (Expr, syntax.Diagnostics) {
	...
	obj, ok := args.(*ObjectExpr)
	if !ok {
		diags := syntax.Diagnostics{ExprError(args, fmt.Sprintf("the argument to %v must be an object containing 'inputs'", kvp.Key.Value()))}
		return RotateSyntax(node, name, args, nil, nil, nil), diags
	}
