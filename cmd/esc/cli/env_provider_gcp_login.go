...
func newEnvProviderGCPLoginOIDCCmd(env *envCommand) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) != 1 {
				return fmt.Errorf("expected <project-number>")
			}
			project, err := strconv.ParseInt(args[0], 10, 64)
			if err != nil {
				return fmt.Errorf("invalid project number %q: must be a positive integer", args[0])
			...
			node := buildGCPLoginOIDCNode(project, workloadPoolID, providerID, serviceAccount, region, tokenLifetime, subjectAttributes)
